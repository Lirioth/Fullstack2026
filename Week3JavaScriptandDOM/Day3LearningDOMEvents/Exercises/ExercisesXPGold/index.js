...
(function() {
  const block = el('section', { class: 'block' }, el('h2', { text: 'Exercise 1: Select a kind of Music 🎵' }));
...
(function() {
  const block = el('section', { class: 'block' }, el('h2', { text: 'Exercise 2: Delete colors 🎨' }));
  ...
  window.removecolor = function removecolor() {
    ...
    if (idx >= 0) {
      ...
      info.textContent = colorSelect.options.length ? `Removed: ${removed} ✅` : 'List is now empty.';
    }
  };
...
(function() {
  const block = el('section', { class: 'block' }, el('h2', { text: 'Exercise 3: Create a shopping list 🛒' }));
  let shoppingList = [];
